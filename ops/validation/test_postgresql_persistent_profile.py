...
CANARY_KUSTOMIZATION = (ROOT / "ops" / "database" / "postgresql" / "canary" / "kustomization.yaml").read_text(encoding="utf-8")
CANARY_NAMESPACE = (ROOT / "ops" / "database" / "postgresql" / "canary" / "namespace.yaml").read_text(encoding="utf-8")
...
class PostgresqlPersistentProfileTests(unittest.TestCase):
    ...
    def test_canary_is_manual_pinned_and_uses_retained_storage(self):
        ...
        self.assertIn("name: faang-postgresql-canary", CANARY_NAMESPACE)
        self.assertNotIn("automated:", CANARY_APPLICATION)
    ...
    def test_backup_configuration_is_private_and_separate_from_existing_s3_identities(self):
        ...
        self.assertIn("AWS_CA_BUNDLE", BACKUP_CONFIGURER)
        self.assertIn("$($configuration.accessKeyFile)", BACKUP_CONFIGURER)
        self.assertIn("$headBucketExitCode = $LASTEXITCODE", BACKUP_CONFIGURER)
        self.assertIn("unable to apply the PostgreSQL backup runtime Secret", BACKUP_CONFIGURER)
        self.assertIn("kind: ObjectStore", BACKUP_CONFIGURER)
        self.assertIn("kind: Namespace", BACKUP_CONFIGURER)
