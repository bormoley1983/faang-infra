...
class KafkaPersistentProfileTests(unittest.TestCase):
    ...
    def test_canary_is_manual_pinned_and_uses_retained_storage(self):
        ...
        self.assertIn("size: 10Gi", CANARY)
        self.assertIn("port: 9092", CANARY)
        self.assertIn("tls: false", CANARY)
        self.assertIn("runAsNonRoot: true", CANARY)
        ...
        self.assertIn('argocd.argoproj.io/sync-wave: "1"', CANARY)
    ...
    def test_canary_has_bounded_resources_and_anti_affinity(self):
        self.assertIn("cpu: 250m", CANARY)
        self.assertIn("memory: 512Mi", CANARY)
        self.assertIn('cpu: "1"', CANARY)
        self.assertIn("memory: 2Gi", CANARY)
        self.assertIn("node-role.kubernetes.io/control-plane", CANARY)
        self.assertIn("operator: DoesNotExist", CANARY)
